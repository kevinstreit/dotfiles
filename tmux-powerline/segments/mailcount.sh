...
export TMUX_POWERLINE_SEG_MAILCOUNT_MAILBOX_TYPE="mailcheck"
...
__count_maildir() {
  for f in chair gmail kevinstreit mailbox; do
    TMUX_POWERLINE_SEG_MAILCOUNT_MAILDIR_INBOX=/home/streit/Mail/$f/INBOX/new/
	  count=$(ls "$TMUX_POWERLINE_SEG_MAILCOUNT_MAILDIR_INBOX" | wc -l)

	  # Fix for mac, otherwise whitespace is left in output
	  if shell_is_osx; then
	  	count=$(echo -n "$count" | sed -e "s/^[ \t]*//")
	  fi
